...
//or-opt法内でのつなぎかえ処理
vector<int> InsertOrOpt(vector<int> const& path, const int& i, const int& j);
//or-opt法
...
vector<int> OrOpt(vector<int> path, vector<vector<double>> const& dist)
{
	...
	while(true){
		cnt = 0;
		for(int i = 1; i < path.size()-2; i++){ //i=0はスタート地点
			for(int j = i+1; j < path.size()-1; j++){ //ゴール地点はswap対象としないのでpath.size()-1まで //j=i+2から始めるのはj=i+1からだとjとi+1を繋ぎ変える際jとi+1が同じになってしまうから
				new_path = InsertOrOpt(path, i, j);
				new_cost = Cost(new_path, dist);
